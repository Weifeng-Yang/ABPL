...
if(flag==0)
[vars,loss,tr,btss,rate]=PALM(var,ngmar,aa,maxiteropt,num,N,stopindex,r);
varss{1}=vars;
lossdata{1}=loss;
trdata{1}=tr;
btz{1}=btss;
...
elseif(flag==1) 
[vars,loss,tr,btss,atz,rate]=IPALM(var,ngmar,aa,maxiteropt,bt,6,num,N,stopindex,r);
varss{1}=vars;
...
[vars,loss,tr,btss,rate]=BPL(var,ngmar,aa,maxiteropt,num,N,stopindex,r);
varss{1}=vars;
lossdata{1}=loss;
trdata{1}=tr;
btz{1}=btss;
...
[vars,loss,tr,btss,ats,rate]=IBPG(var,ngmar,aa,maxiteropt,bt,num,N,stopindex,r);
varss{1}=vars;
lossdata{1}=loss;
trdata{1}=tr;
btz{1}=btss;
...
[vars,loss,tr,btss,rate]=TITAN(var,ngmar,aa,maxiteropt,bt,num,N,stopindex,r);
varss{1}=vars;
lossdata{1}=loss;
trdata{1}=tr;
btz{1}=btss;
...
for i=1:(length(objs))
[vars,loss,tr,btss,rate]=ABPL(var,ngmar,aa,maxiteropt,bt,objs(i),num,N,stopindex,r,t,btmax); %%ABPL-cyclic and ABPL+-cyclic
varss{i}=vars;
lossdata{i}=loss;
trdata{i}=tr;
btz{i}=btss;
...
for i=1:(length(objs))
[vars,loss,tr,btss,rate]=ABPLRD(var,ngmar,aa,maxiteropt,bt,objs(i),num,N,stopindex,r,t,btmax);  %%ABPL-random and ABPL+-random
varss{i}=vars;
lossdata{i}=loss;
trdata{i}=tr;
btz{i}=btss;
