...
if(~isempty(lossdata))
    mess=pltplt(lossdata,trdata,rates,objs,num,trigger(i),mess,color(i));
end
end
end
...
function mess=pltplt(lossdata,trdata,rates,objs,dim,trigger,mess,color)
figure(1)
% color=["-o","-*","-+","-x","-d","-s","-p","-h"];
